...
class CompetitionSetup:
    ...
    def compile_java(self, java_file):
        ...
        try:
            subprocess.check_output(['javac', java_file], stderr=subprocess.STDOUT, cwd=directory)
            return True
    ...
    def run_java(self, class_name, directory):
        ...
        try:
            process = subprocess.run(['java', class_name, self.test_input_file], text=True, capture_output=True, timeout=1000, cwd=directory)
            ...
            for file in files:
                if file.endswith(".java"):
                    ...
                    if self.compile_java(java_file):
                        success, execution_time, output = self.run_java(class_name, directory)
                        ...
                        if success:
                            ...
                            results.append({'student_id': student_id, 'algorithm': class_name, 'dataset': test_input_file_name, 'execution_time': execution_time, 'is_sorted': is_sorted})
                        else:
                            results.append({'student_id': student_id, 'algorithm': class_name, 'dataset': test_input_file_name, 'error': output})
